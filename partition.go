...
// If a sink is full or closed, the value will be discarded.
// If no predicate matches, the value is sent to the default sink (or discarded if full or closed).
func Partition[T any](seq iter.Seq[T], def Sink[T], dests ...PartDest[T]) {
...
vals:
	for val := range seq {
		if doneCount == len(dests)+1 {
			// All sinks are closed, stop processing
			break
		}
	...
	if !defDone {
		def.Close() // Close the default sink if it was not closed during processing
	}
}
